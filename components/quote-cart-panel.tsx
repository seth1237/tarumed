...
export function QuoteCartPanel() {
  ...
  const whatsappHref = () => {
    ...
    const text = `Hello Tarumed, I would like a quote for:\n${lines.join('\n')}`
  ...
  if (items.length === 0) {
    return (
      <div className="quote-empty">
        ...
        <Link href="/shop" className="button button-primary">Browse products</Link>
      </div>
    )
  }
      ...
      <div className="quote-items">
        {items.map((item) => (
          <article key={item.id} className="quote-item">
            <div>
              <Link href={productHref(item)} className="quote-item-name">{item.name}</Link>
              <small>{item.categoryName}</small>
            </div>
            <div className="quote-item-qty">
              <button type="button" aria-label="Decrease quantity" onClick={() => setQuantity(item.id, item.quantity - 1)}>
                <Minus size={14} />
              </button>
              <input
                type="number"
              ...
              />
              <button type="button" aria-label="Increase quantity" onClick={() => setQuantity(item.id, item.quantity + 1)}>
                <Plus size={14} />
              </button>
            </div>
            <span className="quote-item-price">{item.price ? formatKes(item.price * item.quantity) : 'Quote'}</span>
            <button type="button" className="quote-item-remove" aria-label={`Remove ${item.name}`} onClick={() => remove(item.id)}>
              <Trash2 size={16} />
            </button>
          </article>
        ))}
      ...
      <div className="quote-side">
        ...
        <form className="quote-form" onSubmit={onSubmit}>
          <div className="quote-form-head">
            ...
            <p>{items.length} product{items.length === 1 ? '' : 's'} in this quote</p>
          </div>
